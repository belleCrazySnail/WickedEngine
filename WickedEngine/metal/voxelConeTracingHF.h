...
inline float4 ConeTrace(texture3d<float> voxels, float3 P, float3 N, float3 coneDirection, float coneAperture, constant GlobalData &gd)
{
	float3 color = 0;
	float alpha = 0;
	...
	float dist = gd.frame.g_xFrame_VoxelRadianceDataSize; // offset by cone dir so that first sample of all cones are not the same
	float3 startPos = P + N * gd.frame.g_xFrame_VoxelRadianceDataSize * 2 * SQRT2; // sqrt2 is diagonal voxel half-extent

	// We will break off the loop if the sampling distance is too far for performance reasons:
	const float maxDistance = MAX_DIST * gd.frame.g_xFrame_VoxelRadianceDataSize;

	while (dist < maxDistance && alpha < 1)
	{
		float diameter = max(gd.frame.g_xFrame_VoxelRadianceDataSize, 2 * coneAperture * dist);
		float mip = log2(diameter * gd.frame.g_xFrame_VoxelRadianceDataSize_Inverse);
		...
		tc = (tc - gd.frame.g_xFrame_VoxelRadianceDataCenter) * gd.frame.g_xFrame_VoxelRadianceDataSize_Inverse;
		tc *= gd.frame.g_xFrame_VoxelRadianceDataRes_Inverse;
		tc = tc * float3(0.5f, -0.5f, 0.5f) + 0.5f;

		// break if the ray exits the voxel grid, or we sample from the last mip:
		if (any(bool3(tc - saturate(tc))) || mip >= (float)gd.frame.g_xFrame_VoxelRadianceDataMIPs)
			break;
		...
		// step along ray:
		dist += diameter * gd.frame.g_xFrame_VoxelRadianceRayStepSize;
	}
...
// P:				world-space position of receiving surface
// N:				world-space normal vector of receiving surface
inline float4 ConeTraceRadiance(texture3d<float> voxels, float3 P, float3 N, constant GlobalData &gd)
{
	float4 radiance = 0;

	for (uint cone = 0; cone < gd.frame.g_xFrame_VoxelRadianceNumCones; ++cone) // quality is between 1 and 16 cones
	{
		...
		radiance += ConeTrace(voxels, P, N, coneDirection, tan(PI * 0.5f * 0.33f), gd);
	}

	// final radiance is average of all the cones radiances
	radiance *= gd.frame.g_xFrame_VoxelRadianceNumCones_Inverse;
...
inline float4 ConeTraceReflection(texture3d<float> voxels, float3 P, float3 N, float3 V, float roughness, constant GlobalData &gd)
{
	float aperture = tan(roughness * PI * 0.5f * 0.1f);
	float3 coneDirection = reflect(-V, N);

	float4 reflection = ConeTrace(voxels, P, N, coneDirection, aperture, gd);
