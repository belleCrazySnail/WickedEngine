...
void wiOcean::Render(const CameraComponent& camera, const WeatherComponent& weather, float time, GRAPHICSTHREAD threadID)
{
    ...
    cb.xOceanSurfaceDisplacementTolerance = std::max(1.f, params.surfaceDisplacementTolerance);
...
void wiOcean::LoadShaders()
{
	...
	m_pUpdateSpectrumCS = static_cast<ComputeShader*>(wiResourceManager::GetShaderManager().add("oceanSimulatorCS", wiResourceManager::COMPUTESHADER));
	m_pUpdateDisplacementMapCS = static_cast<ComputeShader*>(wiResourceManager::GetShaderManager().add("oceanUpdateDisplacementMapCS", wiResourceManager::COMPUTESHADER));
	m_pUpdateGradientFoldingCS = static_cast<ComputeShader*>(wiResourceManager::GetShaderManager().add("oceanUpdateGradientFoldingCS", wiResourceManager::COMPUTESHADER));


	g_pOceanSurfVS = static_cast<VertexShader*>(wiResourceManager::GetShaderManager().add("oceanSurfaceVS", wiResourceManager::VERTEXSHADER));

	g_pOceanSurfPS = static_cast<PixelShader*>(wiResourceManager::GetShaderManager().add("oceanSurfacePS", wiResourceManager::PIXELSHADER));
	g_pWireframePS = static_cast<PixelShader*>(wiResourceManager::GetShaderManager().add("oceanSurfaceSimplePS", wiResourceManager::PIXELSHADER));
